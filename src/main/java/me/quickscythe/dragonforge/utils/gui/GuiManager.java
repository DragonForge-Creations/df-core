package me.quickscythe.dragonforge.utils.gui;
...
public class GuiManager {
    ...
    public static void init() {
        guiFolder = new File(CoreUtils.plugin().getDataFolder().getPath() + "/guis");
        registerGuis();
...
//        player.openInventory(gui.getInventory(player));
    ...
    public static void switchGui(Player player, final GuiInventory gui) {
        if (gui == null) return;
        closeGui(player);
        player.setMetadata("switchinv", new FixedMetadataValue(CoreUtils.plugin(), "yup"));
//        player.openInventory(getGuis().get(invTracker.get(player.getUniqueId())).getInventory(player));
        ...
        Bukkit.getScheduler().runTaskLater(CoreUtils.plugin(), new Runnable() {

            @Override
            public void run() {
                invTracker.remove(player.getUniqueId());
                player.removeMetadata("switchinv", CoreUtils.plugin());
                openGui(player, gui);
            }
    ...
    public static void closeGui(Player player) {
        if (invTracker.containsKey(player.getUniqueId())) {
            player.setMetadata("switchinv", new FixedMetadataValue(CoreUtils.plugin(), true));
    ...
    private static void loadGuis(File file) {
        JSONObject json = (JSONObject) DataManager.getStorage().load(file);
        for (String name : json.keySet()) {
            JSONObject gui = json.getJSONObject(name);
            GuiInventory inv = new GuiInventory(name, gui.getString("name"), gui.getInt("size"), gui.getString("config"));
            for (String item : gui.getJSONObject("items").keySet()) {
                JSONObject i = gui.getJSONObject("items").getJSONObject(item);
                GuiItem gi = new GuiItem(item);
                gi.setDisplayName(i.getString("name"));
                gi.setMaterial(Material.valueOf(i.getString("type")));
                if (i.has("lore")) {
                    List<String> lore = new ArrayList<>();
                    for (int x = 0; x != i.getJSONArray("lore").length(); x++)
                        lore.add(i.getJSONArray("lore").getString(x));
                    gi.setLore(lore);
                }
                if(i.has("actions")) {
                    JSONArray actions = i.getJSONArray("actions");
